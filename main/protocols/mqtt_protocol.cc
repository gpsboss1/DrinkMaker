...
bool MqttProtocol::OpenAudioChannel()
{
    ...
    udp_->OnMessage([this](const std::string &data)
        ...
        // 打印UDP包信息Print packet sizes for debugging: total UDP size, payload length in header, and estimated decrypted size
        // ESP_LOGI(TAG, "Audio packet sizes: total=%u, header_payload_len=%u, decrypted_est=%u, session_id=%s, seq=%lu, ts=%lu",
        // (unsigned int)data.size(),
        // (unsigned int)ntohs(*(uint16_t*)&data[2]),
        // (unsigned int)(data.size() - aes_nonce_.size()),
        // session_id_.c_str(),
        // sequence,
        // timestamp);
